...
class AStarEuclideanPlanner(CellBasedForwardSearch):
    ...
    def insert(self, cell):
        if len(self.fifoQueue) == 0:
            ...
            for i in range(len(self.fifoQueue)):
                ...
                if cost + self.euclidean_heuristic(cell) <= self.fifoQueue[i].pathCost + self.euclidean_heuristic(self.fifoQueue[i]):
                    index = i
