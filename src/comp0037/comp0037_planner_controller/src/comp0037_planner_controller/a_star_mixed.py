...
class AStarMixedPlanner(CellBasedForwardSearch):
    ...
    def mixed_heuristic(self, cell):
        """
        0.5*Euclidean + 0.5*Manhatton
        """
        dx = abs(cell.coords[0]-self.goal.coords[0])
        dy = abs(cell.coords[1]-self.goal.coords[1])

        return 0.5*np.sqrt(dx*dx + dy*dy) + 0.5*(dx + dy)
    ...
    def insert(self, cell):
        if len(self.fifoQueue) == 0:
            ...
            for i in range(len(self.fifoQueue)):
                ...
                if cost + self.mixed_heuristic(cell) <= self.fifoQueue[i].pathCost + self.mixed_heuristic(self.fifoQueue[i]):
                    index = i
